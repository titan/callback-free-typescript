...
function convert_to_callback(vardecl, call, statements) {
    var block = ts.createBlock(statements);
    var parameters = vardecl.declarationList.declarations.map(function (decl) { return ts.createParameter([], [], null, decl.getChildAt(0).getText(), null, decl.getChildCount() > 2 ? ts.createTypeReferenceNode(decl.getChildAt(2).getText(), []) : null); });
    ...
    for (var i = 0, len = statements.length; i < len; i++) {
        ...
        if (i > 0 && i < len - 1 && stmt.kind === ts.SyntaxKind.MissingDeclaration) {
            ...
            if (pstmt.kind === ts.SyntaxKind.VariableStatement && nstmt.kind === ts.SyntaxKind.ExpressionStatement && nstmt.expression.kind === ts.SyntaxKind.CallExpression) {
                ...
                var stmts = null;
                if (i > 1) {
                    stmts = statements.slice(0, i - 1).concat([convert_to_callback(pstmt, nstmt.expression, iterate_statements(statements.slice(i + 2)))]);
                }
                else {
                    stmts = [convert_to_callback(pstmt, nstmt.expression, iterate_statements(statements.slice(i + 2)))];
                }
                return stmts;
            }
        }
